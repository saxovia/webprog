...
interface IStorage {
     ...
     */
    function add($record, $num): string;

    function addUser($record, $num): string;
...
class Storage implements IStorage {
     ...
     * @return string id assigned to the inserted entry
     */
    public function add($record, $num): string {
        if (is_array($record)) {
            $record['id'] = $num;
        } elseif (is_object($record)) {
            $record->id = $num;
        }
        $this->contents["card{$num}"] = $record;
        return $num;
    }
    public function addUser($record, $num): string {
        if (is_array($record)) {
            $record['id'] = $num;
        } elseif (is_object($record)) {
            $record->id = $num;
        }
        $this->contents["{$num}"] = $record;
        return $num;
    }
